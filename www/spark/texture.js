...
spark.Frame = function(x1, y1, x2, y2, u1, v1, u2, v2) {
...
spark.Image = function(src) {
  ...
  // Default frame.
  this.frame = new spark.Frame(-1, -1, 1, 1, 0, 0, 1, 1);
  ...
  this.source.onload = (function() {
    ...
    this.width = spark.nextPow2(this.source.width);
    this.height = spark.nextPow2(this.source.height);
...
// A texture atlas.
spark.Atlas = function(src) {
  // TODO:
};

// A font atlas.
spark.Font = function(src) {
  this.pages = [];
  this.glyphs = [];
  ...
  spark.loadXML(src, (function(font) {
    ...
    for(var i = 0;i < pages.length;i++) {
      ...
      this.pages[id] = new spark.Image(src);
    }
...
// Set constructors.
spark.Frame.prototype.constructor = spark.Frame;
spark.Image.prototype.constructor = spark.Image;
spark.Atlas.prototype.constructor = spark.Atlas;

// Bind a frame to be rendered.
spark.Frame.prototype.bind = function() {

  // Bind the vertices.
  gl.bindBuffer(gl.ARRAY_BUFFER, this.quadBuffer);
  gl.bufferData(gl.ARRAY_BUFFER, this.quad, gl.STATIC_DRAW);
  gl.vertexAttribPointer(spark.currentShader.a.position, 2, gl.FLOAT, false, 0, 0);
  gl.enableVertexAttribArray(spark.currentShader.a.position);

  // Bind the texture coordinates.
  gl.bindBuffer(gl.ARRAY_BUFFER, this.uvBuffer);
  gl.bufferData(gl.ARRAY_BUFFER, this.uvs, gl.STATIC_DRAW);
  gl.vertexAttribPointer(spark.currentShader.a.uv, 2, gl.FLOAT, false, 0, 0);
  gl.enableVertexAttribArray(spark.currentShader.a.uv);
};

// Set the pivot of an image. This alters the quad.
spark.Image.prototype.setPivot = function(x, y) {
...
spark.Image.prototype.blit = function(frame) {
  gl.activeTexture(gl.TEXTURE0);
  gl.bindTexture(gl.TEXTURE_2D, this.texture);
  gl.uniform1i(spark.currentShader.u.sampler, 0);
...
spark.Atlas.prototype.blit = function(frame) {
  this.image.blit(this.frames[frame]);
};

// Render a font string.
spark.Font.prototype.drawText = function(string, x, y) {
